      ...
      <div className="space-y-3" id="progress-chart-container">
        <div className="flex items-center justify-between text-xs text-[#F0F0F0] uppercase font-bold tracking-widest italic">
          <span className="flex items-center gap-1">
            <LineChart className="text-[#EFE71D]" size={13} />
            Evolução de Cargas Acumuladas
          </span>
          <span className="text-[#EFE71D] font-mono text-[9px] font-black leading-none">+12.5% vs Mês Ant.</span>
        </div>
            ...
            <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto overflow-visible">
              ...
              {points.map((p, i) => (
                <g key={i} className="group cursor-pointer">
                  <circle cx={p.x} cy={p.y} r="4.5" fill="#EFE71D" stroke="#050505" strokeWidth="1.5" />
                  <circle cx={p.x} cy={p.y} r="8" fill="#EFE71D" opacity="0.2" className="hover:opacity-50 transition" />
                  
                  {/* Floating tooltip simulation */}
                  <text x={p.x} y={p.y - 10} fill="#EFE71D" fontSize="8" fontWeight="bold" fontFamily="monospace" textAnchor="middle">
                    {p.value}kg
                  </text>
              ...
              <defs>
                <linearGradient id="chart-gradient" x1="0" y1="0" x2="1" y2="0">
                  <stop offset="0%" stopColor="#EFE71D" />
                  <stop offset="100%" stopColor="#22c55e" />
                </linearGradient>
                <linearGradient id="chart-glow" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#EFE71D" />
                  <stop offset="100%" stopColor="#EFE71D" stopOpacity="0" />
                </linearGradient>
              </defs>
            </svg>
          </div>
  ...
  return (
    <div className="space-y-6 animate-fadeIn" id="student-workout-dashboard">
      ...
      <div className="bg-[#121212] border border-[#222] rounded-3xl p-6 sm:p-8 space-y-6 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-96 h-96 bg-[#EFE71D] opacity-[0.015] blur-[120px] pointer-events-none" />
        
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-black text-white uppercase italic tracking-tight flex items-center gap-2">
              <Calendar className="text-[#EFE71D]" size={18} />
              CRONOGRAMA DE TREINOS DA SEMANA
            </h3>
            <p className="text-xs text-neutral-250 font-medium mt-1">
              Defina quais dias treinar ou descansar. Toque em qualquer dia da semana para abrir e visualizar a ficha correspondente.
            </p>
          </div>
          <div className="bg-black/45 border border-[#222]/60 px-4 py-2.5 rounded-xl text-xs text-neutral-200 font-mono font-extrabold shadow-sm">
            HOJE: <span className="text-[#EFE71D]">{realTodayName}-FEIRA</span>
          </div>
        </div>
            ...
            return (
              <div
                ...
                className={`text-left p-4 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between h-36 relative group/day ${
                  isSelected 
                    ? 'bg-black border-[#EFE71D] shadow-md shadow-[#EFE71D]/5' 
                    : 'bg-black/40 border-[#222]/60 hover:border-neutral-700 hover:bg-black/60'
                } ${isToday ? 'ring-1 ring-[#EFE71D]/40' : ''}`}
                title={`Ver ficha técnica de ${day}`}
              >
                {/* Day Header */}
                <div className="flex items-center justify-between gap-1 mb-2">
                  <span className={`text-xs font-black uppercase tracking-wider ${isToday ? 'text-[#EFE71D]' : 'text-neutral-200'}`}>
                    {day}
                  </span>
                  {isToday && (
                    <span className="text-[8px] bg-[#EFE71D] text-black px-1.5 py-0.5 rounded-sm font-black uppercase tracking-widest leading-none font-mono">
                      HOJE
                    </span>
                  )}
                </div>
                ...
                <div className="mt-1 mb-3" onClick={(e) => e.stopPropagation()}>
                  <select
                    ...
                    className="w-full bg-[#121212] border border-[#222] hover:border-[#EFE71D]/40 text-xs font-bold text-white rounded-lg px-2 py-1.5 focus:outline-none transition font-sans cursor-pointer focus:ring-1 focus:ring-[#EFE71D]/20"
                  >
                ...
                {/* Status Indicator */}
                <div className="flex items-center justify-between text-[10px] font-mono mt-auto pt-2 border-t border-[#222]/30">
                  {sched.planId === 'rest' ? (
                    <span className="text-neutral-400 uppercase font-black tracking-wider leading-none">OFF</span>
                  ) : isCompletedToday ? (
                    <span className="text-[#EFE71D] font-black flex items-center gap-0.5 uppercase tracking-tight">
                      <Check size={11} className="stroke-[3]" /> CONCLUÍDO
                    </span>
                  ) : (
                    <span className="text-neutral-300 font-extrabold uppercase tracking-tight">PENDENTE</span>
                  )}
                </div>
              </div>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-[#121212] border border-[#222] rounded-3xl p-8 relative overflow-hidden group">
            ...
            <div className="absolute top-0 right-0 w-64 h-64 bg-[#EFE71D] opacity-[0.03] blur-[100px] pointer-events-none" />
            
            {currentPlan ? (
              <div className="flex flex-col sm:flex-row justify-between items-start gap-6">
                <div className="space-y-3">
                  <span className="text-[#EFE71D] text-xs font-black tracking-[0.2em] uppercase block italic flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#EFE71D] animate-pulse" />
                    Roteiro para {selectedDay}-feira {selectedDay === realTodayName ? '(Hoje)' : ''}
                  </span>
                  
                  {daySchedule.completedAt === new Date().toISOString().split('T')[0] && (
                    <span className="inline-flex items-center gap-1 bg-[#EFE71D]/10 border border-[#EFE71D]/30 text-[#EFE71D] text-xs font-black px-2.5 py-1 rounded-md uppercase tracking-wider">
                      <Check size={11} className="stroke-[3]" /> Treino do dia concluído!
                    </span>
                  )}
                  ...
                  <h2 className="text-3xl sm:text-4xl font-black leading-tight uppercase italic text-white tracking-tighter">
                    ...
                    <span className="text-[#EFE71D]">{currentPlan.title.split(' ').slice(1).join(' ') || 'Hipertrofia'}</span>
                  </h2>
                  <p className="text-neutral-200 max-w-lg text-sm font-medium leading-relaxed">
                ...
                <div className="w-full sm:w-auto shrink-0 flex flex-col gap-2.5">
                  <button
                    ...
                    className="w-full sm:w-auto bg-[#EFE71D] hover:bg-white text-black px-8 py-4 rounded-xl font-black uppercase italic tracking-tighter transition-colors cursor-pointer text-center text-sm shadow-lg shadow-[#EFE71D]/5 hover:text-black"
                  >
                    {daySchedule.completedAt === new Date().toISOString().split('T')[0] ? 'Refazer Treino' : 'Iniciar Treino'}
                  </button>
                  {selectedDay !== realTodayName && (
                    <span className="text-xs text-center text-neutral-300 font-mono font-bold italic block">
              ...
              <div className="py-6 text-center space-y-4">
                <div className="w-12 h-12 bg-neutral-900 border border-[#222] rounded-full flex items-center justify-center mx-auto text-[#EFE71D] text-xl">
                  🧘
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-neutral-200 max-w-sm mx-auto leading-relaxed font-medium">
                ...
                <div className="bg-black/40 border border-[#222]/55 p-5 rounded-2xl max-w-md mx-auto text-left space-y-3.5 mt-4">
                  <span className="text-xs text-[#EFE71D] font-mono font-black uppercase tracking-wider block">QUER TREINAR MESMO ASSIM?</span>
                  <p className="text-xs text-neutral-200 font-semibold leading-relaxed">
                    Você pode ignorar o cronograma hoje e realizar qualquer uma das sessões catalogadas do Mendes Fitness:
                  </p>
                  <div className="grid grid-cols-1 gap-2">
                    {SUGGESTED_PLANS.map(p => (
                      ...
                      >
                        ...
                        <Play size={10} className="text-[#EFE71D] group-hover:translate-x-1 transition-transform" />
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {currentPlan && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8 pt-6 border-t border-[#222] text-xs">
                <div className="bg-black/40 p-3 rounded-xl border border-[#222]/60">
                  <p className="text-[10px] text-neutral-300 uppercase font-black tracking-widest leading-none mb-1.5">Ambiente</p>
                  <p className="text-white font-black uppercase italic tracking-tighter text-sm">{currentPlan.location}</p>
                </div>
                <div className="bg-black/40 p-3 rounded-xl border border-[#222]/60">
                  <p className="text-[10px] text-neutral-300 uppercase font-black tracking-widest leading-none mb-1.5">Objetivo</p>
                  <p className="text-white font-black uppercase italic tracking-tighter text-sm">{currentPlan.goal}</p>
                </div>
                <div className="bg-black/40 p-3 rounded-xl border border-[#222]/60">
                  <p className="text-[10px] text-neutral-300 uppercase font-black tracking-widest leading-none mb-1.5">Nível Técnico</p>
                  <p className="text-[#EFE71D] font-black uppercase italic tracking-tighter text-sm">{currentPlan.level}</p>
                </div>
                <div className="bg-black/40 p-3 rounded-xl border border-[#222]/60">
                  <p className="text-[10px] text-neutral-300 uppercase font-black tracking-widest leading-none mb-1.5">Freq. Semanal</p>
          ...
          {currentPlan && (
            <div className="space-y-3">
              <h3 className="text-xs font-black text-neutral-200 uppercase tracking-widest pl-1 italic">
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="w-8 h-8 bg-black border border-[#222] text-[#EFE71D] rounded-xl flex items-center justify-center font-mono font-black text-sm italic">
                          {String(idx + 1).padStart(2, '0')}
                        </span>
                        <div>
                          <h4 className="font-bold text-sm text-white">{ex.name}</h4>
                          <div className="flex items-center gap-1.5 mt-1.5">
                            <span className="text-[10px] text-[#EFE71D] font-black uppercase tracking-wider bg-black/40 px-2 py-0.5 rounded border border-[#EFE71D]/20">
                              {ex.muscleGroup}
                            </span>
                      ...
                      <div className="text-neutral-300 hover:text-white transition">
                    ...
                    {isExpanded && (
                      <div className="p-5 bg-black/30 border-t border-[#222] text-xs text-neutral-200 leading-relaxed space-y-4 animate-fadeIn">
                        <div className="space-y-1.5">
                          <p className="font-black text-[10px] text-[#EFE71D] uppercase tracking-widest flex items-center gap-1.5">
                            <HelpCircle size={12} className="text-[#EFE71D]" />
                            Guia Completo de Execução Segura
                          </p>
                          <p className="text-neutral-300 leading-relaxed text-xs font-semibold">
                            {ex.instructions}
                          </p>
                        </div>

                        <div className="flex gap-2 text-[10px] font-mono text-neutral-400 font-bold">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#121212] border border-[#222] rounded-3xl p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-black text-neutral-200 uppercase tracking-widest italic">Histórico de Treinos</h4>
              <Calendar className="text-neutral-200" size={14} />
            </div>

            <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
              {logs.length === 0 ? (
                <div className="text-center py-8 text-neutral-300 text-xs font-medium">
                ...
                [...logs]
                  ...
                  .map((log) => (
                    <div
                      key={log.id}
                      className="p-3.5 bg-black/40 rounded-xl border border-[#222] flex items-center justify-between text-xs hover:border-[#EFE71D]/20 transition"
                    >
                      <div className="space-y-1">
                        <p className="font-bold text-white text-xs truncate max-w-[150px]">{log.workoutTitle}</p>
                        <span className="text-[11px] text-neutral-300 font-mono font-medium">
                          {log.date} • {log.completedSetsCount} séries
                        </span>
                      </div>

                      <div className="text-right">
                        <span className="font-mono text-xs font-black text-[#EFE71D] block animate-pulse">
                          +{log.totalWeightLifted}
                        </span>
                        <span className="text-[10px] text-neutral-300 font-mono uppercase font-black tracking-wider">kg totais</span>
                      </div>
                    </div>
                  ))
              )}
